...
export default function SwipeList() {  
  ...
  const [items, setItems] = useState<{ id: number; key: string; text: string }[]>([]);
  const [inputValue, setInputValue] = useState('');

  const handleAddItem = () => {
    if (inputValue.trim()) {
      const newItem = { id: items.length, key: `${items.length}`, text: inputValue };
  ...
  const deleteItem = (item: { key: string; text: string; }) => {
...
const renderItem = ({ item }: { item: {
  ...
  const renderHiddenItem = (data: { item: {
      id: any; key: string; text: string; 
}; }, rowMap: any) => (
